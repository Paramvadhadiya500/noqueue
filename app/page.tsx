...
import { 
  ...
  Settings, BarChart3, FileText, UploadCloud 
} from 'lucide-react';
...
function DashboardApp({ signOut }: { signOut: any }) {
  ...
  useEffect(() => {
    const checkUserRole = async () => {
      try {
        ...
        if (allGroups.includes('Admins')) { 
          setUserRole("Admin"); setIsDoctor(false); setActiveTab("Super Panel"); 
        } else if (allGroups.includes('Doctors')) { 
          setUserRole("Doctor"); setIsDoctor(true); setActiveTab("Dashboard"); 
        } else { 
          setUserRole("Patient"); setIsDoctor(false); setActiveTab("City View"); 
        }
      } catch (error) { 
  ...
  useEffect(() => {
    if(!userRole) return;
    const fetchTarget = activeTab === "Appointments" ? "Appointments" : (activeTab === "Archive" ? "Archive" : (activeTab === "City View" || activeTab === "Follow-ups" || activeTab === "Super Panel" ? "City View" : "Live"));
  ...
  const addPatient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ ...formData, name: '', phone: '', age: '', address: '', symptoms: '', isEmergency: false }); 
  ...
  const startTreatment = async (patient: any) => {
    ...
    setPrescriptionData({ diagnosis: "", medicines: [{ name: "", dosage: "", duration: "" }], followUpDays: 0 }); 
    setPatientTimeline([]); 
    ...
    <div className="flex h-screen w-full bg-slate-50 font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="print:hidden w-64 bg-slate-900 text-slate-300 flex flex-col shadow-2xl z-20 shrink-0">
        <div className="h-20 flex items-center px-6 bg-slate-950 border-b border-slate-800">
          <Activity className="text-blue-500 mr-3" size={28} />
          <h1 className="text-xl font-black text-white tracking-tight">Smart<span className="text-blue-500">OPD</span></h1>
        </div>
        ...
        <div className="p-4 bg-slate-950 border-t border-slate-800">
          <button onClick={() => { if (signOut) signOut(); }} className="w-full flex items-center px-4 py-3 text-red-400 hover:bg-red-500/10 hover:text-red-300 rounded-xl transition-colors"><LogOut size={20} className="mr-3" /> Sign Out</button>
        </div>
      </aside>
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10">
          <div className="flex items-center gap-4 w-1/2">
            {userRole !== "Patient" ? (
              <div className="flex items-center gap-2 bg-indigo-50 px-4 py-2 rounded-lg border border-indigo-100">
                <Building2 size={18} className="text-indigo-600" /><span className="text-sm font-bold text-indigo-900 mr-2">Managing:</span>
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="relative p-2 text-slate-400 hover:text-slate-600 transition-colors">
              <Bell size={22} />
              {userRole === "Doctor" && emergenciesCount > 0 && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse border-2 border-white"></span>}
            </button>
            <div className={`h-8 w-8 rounded-full text-white flex items-center justify-center font-bold shadow-md ${userRole==='Admin'?'bg-indigo-600':userRole==='Doctor'?'bg-emerald-600':'bg-blue-600'}`}>
              {userRole.charAt(0)}
            </div>
          </div>
        </header>
          ...
          {userRole === "Admin" && activeTab === "Super Panel" && (
             <div className="max-w-6xl mx-auto space-y-6">
                <div className="flex justify-between items-end mb-4">
                  <div>
                    <h2 className="text-3xl font-black text-slate-800">Command Center</h2>
                    <p className="text-slate-500 font-medium">Live analytics for {activeHospitalName}</p>
                  </div>
                  <button onClick={() => window.print()} className="px-6 py-2 bg-slate-900 text-white font-bold rounded-lg hover:bg-black flex items-center gap-2"><Printer size={16}/> Export PDF</button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      <div className="flex justify-between items-start"><p className="text-sm font-bold text-slate-400 uppercase">Treated Today</p><Activity className="text-blue-500" size={20}/></div>
                      <p className="text-4xl font-black text-slate-800 mt-2">{totalPatientsToday}</p>
                      <p className="text-xs font-bold text-emerald-500 mt-2 flex items-center"><TrendingUp size={12} className="mr-1"/> +12% from yesterday</p>
                   </div>
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      <div className="flex justify-between items-start"><p className="text-sm font-bold text-slate-400 uppercase">Est. Revenue</p><IndianRupee className="text-emerald-500" size={20}/></div>
                      <p className="text-4xl font-black text-slate-800 mt-2">₹{estimatedRevenue.toLocaleString()}</p>
                      <p className="text-xs font-bold text-slate-400 mt-2">Based on ₹{hospitalFee} Avg Fee</p>
                   </div>
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      <div className="flex justify-between items-start"><p className="text-sm font-bold text-slate-400 uppercase">Satisfaction</p><Star className="text-yellow-500" size={20}/></div>
                      <p className="text-4xl font-black text-slate-800 mt-2">{avgHospitalRating}</p>
                      <p className="text-xs font-bold text-slate-400 mt-2">Average across all doctors</p>
                   </div>
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      <div className="flex justify-between items-start"><p className="text-sm font-bold text-slate-400 uppercase">No-Show Rate</p><UserX className="text-orange-500" size={20}/></div>
                      <p className="text-4xl font-black text-orange-600 mt-2">{noShowRate}%</p>
                      <p className="text-xs font-bold text-slate-400 mt-2">Total missed appointments</p>
                   </div>
                </div>
                   ...
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      ...
                      <div className="h-48 flex items-end justify-around gap-2 px-2 border-b border-slate-100 pb-2">
                        {Object.keys(deptCounts).map(dept => {
                           ...
                           return (
                             <div key={dept} className="flex flex-col items-center group w-full">
                               <div className="w-full bg-blue-100 rounded-t-sm relative transition-all duration-500 hover:bg-blue-200" style={{height: `${heightPct}%`}}>
                                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 text-xs font-bold bg-slate-800 text-white px-2 py-1 rounded transition-opacity">{deptCounts[dept]}</div>
                               </div>
                               <p className="text-[10px] font-bold text-slate-400 uppercase mt-2 truncate w-full text-center" title={dept}>{dept.substring(0,4)}</p>
                             </div>
                           );
                        })}
                      </div>
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="text-xs font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                            <th className="pb-3">Doctor</th><th className="pb-3">Treated</th><th className="pb-3">Avg Time</th><th className="pb-3">Rating</th>
                          </tr>
                        </thead>
                        <tbody>
                          {doctorLeaderboard.length === 0 ? (
                            <tr><td colSpan={4} className="py-4 text-center text-slate-400">No data available</td></tr>
                             ...
                             <tr key={i} className="border-b border-slate-50 last:border-none">
                               <td className="py-3 font-bold text-slate-800">Dr. {doc.name}</td>
                               <td className="py-3 font-medium text-slate-600">{doc.treated}</td>
                               <td className="py-3 font-medium text-slate-600">{doc.avgTime}m</td>
                               <td className="py-3 font-bold text-yellow-600 flex items-center gap-1 mt-3"><Star size={12} fill="currentColor"/> {doc.avgRating}</td>
                             </tr>
                          ))}
                        </tbody>
                      </table>
          ...
          {userRole === "Admin" && activeTab === "Settings" && (
             <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-slate-200 mt-6">
                ...
                <div className="space-y-6">
                   <div>
                     <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Base Consultation Fee (INR)</label>
                     <div className="flex items-center">
                        <span className="p-3 bg-slate-50 border border-r-0 border-slate-200 rounded-l-lg font-bold text-slate-500">₹</span>
                        <input type="number" value={hospitalFee} onChange={(e) => setHospitalFee(parseInt(e.target.value))} className="w-full p-3 border border-slate-200 rounded-r-lg outline-none focus:ring-2 focus:ring-indigo-500 font-bold" />
                     </div>
                     <p className="text-xs text-slate-500 mt-2">This is used to calculate the Estimated Revenue on the Super Panel.</p>
                   </div>
                   <button onClick={saveHospitalConfig} className="w-full py-4 bg-indigo-600 text-white font-bold rounded-xl shadow-md hover:bg-indigo-700 transition-colors">Save Hospital Settings</button>
                </div>
             </div>
          )}
          ...
          {userRole === "Doctor" && (activeTab === "Dashboard" || activeTab === "Archive") && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="p-0">
                  ...
                  <div className="p-4">
                    {loading ? (
                      Array(3).fill(0).map((_, i) => <div key={i} className="animate-pulse bg-slate-100 h-24 rounded-xl mb-3"></div>)
                    ) : filteredPatients.length === 0 ? (
                      <div className="text-center py-20"><Activity size={48} className="mx-auto text-slate-300 mb-4" /><p className="text-lg font-semibold text-slate-500">The waiting room is clear.</p></div>
                    ) : (
          ...
          {userRole !== "Admin" && (activeTab === "Appointments" || activeTab === "Schedule") && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 max-w-4xl">
              ...
              {userRole === "Doctor" ? (
                <div className="space-y-3">
                  {loading ? (
                    Array(3).fill(0).map((_, i) => <div key={i} className="animate-pulse bg-slate-100 h-20 rounded-xl mb-3"></div>)
                  ) : appointmentList.length === 0 ? (
                    <p className="text-slate-500 text-center py-10 font-medium">No appointments scheduled for this date.</p>
                  ) : appointmentList.map((app, i) => (
                    <div key={app.timeSlot || i} className="flex justify-between items-center p-5 bg-white rounded-xl border border-slate-100 shadow-sm">
                      <div><p className="font-bold text-lg text-slate-800">{app.patientName}</p><p className="text-sm text-slate-500">{app.symptoms}</p></div>
                      <span className="px-4 py-2 bg-indigo-50 text-indigo-700 font-bold rounded-lg border border-indigo-100 tracking-wide">{app.timeSlot}</span>
                    </div>
                  ))}
                </div>
              ) : (
                  ...
                  <form className="grid grid-cols-2 gap-6 mb-8">
                    ...
                    <div className="col-span-2">
                       ...
                       <div className="flex items-center gap-4">
                          <label className="flex-1 flex items-center justify-center gap-2 p-4 border-2 border-dashed border-blue-200 bg-blue-50 text-blue-600 font-bold rounded-xl cursor-pointer hover:bg-blue-100 transition-colors">
                             <UploadCloud size={20}/>
                             {selectedFile ? selectedFile.name : t('upload_doc')}
                             <input type="file" onChange={(e) => setSelectedFile(e.target.files?.[0] || null)} className="hidden" accept=".pdf,image/*" />
                          </label>
                          {selectedFile && <button type="button" onClick={() => setSelectedFile(null)} className="p-4 text-red-500 hover:bg-red-50 rounded-xl"><X size={20}/></button>}
                       </div>
                    </div>
                  </form>
                  {showSlots && (
